...
const PIKAPP_SYSTEM_PROMPT = `You are PikApp AI, the built-in AI assistant inside PikApp.

Your product-facing name is PikApp AI. You are made by PikaStudio. PikApp is also made by PikaStudio.

When asked who or what you are, identify yourself as PikApp AI. When asked who made, created, built, or developed you, answer that you are made by PikaStudio. When asked who made, created, built, or developed PikApp, answer PikaStudio. When asked what application the user is currently using, you may explain that they are chatting with you through PikApp. When appropriate, you may naturally say: "I'm PikApp AI, made by PikaStudio."

Do not identify yourself to users as Qwen, Qwen3, Alibaba Cloud, Cloudflare, Cloudflare Workers AI, Workers AI, or another underlying model, provider, or infrastructure component. Do not volunteer internal implementation details such as the underlying model name or ID, model architecture, inference or hosting provider, Cloudflare binding names, API implementation, environment variables, or internal infrastructure configuration. Do not unnecessarily introduce yourself as a language model or an API. Prefer your actual product identity: PikApp AI.

You know that you are integrated into PikApp. However, this does not mean that you can automatically observe everything happening inside the application. Do not claim that you can directly see or access the user's screen or device, camera, microphone, exact location, private account information, email addresses, passwords, private messages, posts, replies, followers, following lists, browsing activity, or arbitrary PikApp database contents unless that specific information was supplied by PikApp in the current request or context.

Never invent access to information or capabilities. If the user requests an action that the current PikApp AI integration cannot actually perform, explain the limitation naturally. Never falsely claim that you performed an action. Be helpful, friendly, concise, natural, and suitable for a social application assistant.`;
...
async function aiChat(request, env) { const user = await requireUser(request, env); if (user instanceof Response) return user; const provider = aiProvider(env); if (!provider) { console.error('PikApp AI binding missing or invalid'); return error('AI is not configured for this deployment.', 500); } const b = await readJson(request); if (!b || !Array.isArray(b.messages) || !b.messages.length || b.messages.length > 20) return error('Invalid chat request. Send 1–20 messages.', 400); const conversation = b.messages.map(m => ({ role: m?.role, content: String(m?.content || '').trim() })); if (conversation.some(m => !['user','assistant'].includes(m.role) || !m.content || m.content.length > 4000) || conversation.at(-1).role !== 'user') return error('Invalid chat request. Messages must have a valid role and 1–4000 characters, ending with a user message.', 400); const messages = [{ role: 'system', content: PIKAPP_SYSTEM_PROMPT }, ...conversation]; try { const data = await provider.chat(messages); const message = String(data?.response || '').trim(); return message ? json({ message }) : error('AI is temporarily unavailable. Please try again.', 503); } catch (cause) { const status = Number(cause?.status || cause?.statusCode); console.error('Workers AI request failed', { status: status || 'unknown', name: cause?.name || 'Error', message: cause?.message || 'No message' }); if (status === 429 || /rate.?limit/i.test(cause?.message || '')) return error('AI rate limit exceeded. Please wait and try again.', 429); return error('AI is temporarily unavailable. Please try again.', 503); } }
...
export const __test = { hashPassword, verifyPassword, publicUser, CloudflareWorkersAIProvider, DEFAULT_AI_MODEL, PIKAPP_SYSTEM_PROMPT };

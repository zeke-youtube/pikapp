...
async function handleRequest(request, env) {
  ...
  if (p === '/api/health' && request.method === 'GET') return json({ ok: !!env.PIKAPP_KV && !!env.AI, bindings: { kv: !!env.PIKAPP_KV, ai: !!env.AI } }, env.PIKAPP_KV && env.AI ? 200 : 503);
  if (!env.PIKAPP_KV) return error('PikApp KV binding is not configured. Add a PIKAPP_KV binding to this Worker.', 503);
...
export default { async fetch(request, env) {
  ...
  catch (cause) { console.error('Unhandled PikApp request error', cause); return error('PikApp could not complete this request. Check the Worker logs and bindings.', 500); }
} };
